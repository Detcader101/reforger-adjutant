...
class EventManageView(view_util.ErrorHandledView):
    ...
    def __init__(self, cog: "EventsCog", guild: discord.Guild, invoker_id: int,
                 events: list[events_service.Event], timeout: float = 180.0):
        ...
        self.event_select.options = options or [discord.SelectOption(label="Nothing on the books", value="__none__")]
    ...
    async def teardown_button(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        if self.selected_id is not None:
...
class EventsCog(commands.Cog, name="EventsCog"):
    ...
    @app_commands.command(name="event", description="Show upcoming events, or create one by name and start time.")
    ...
    @rate_limited()
    async def event(
        self, interaction: discord.Interaction, name: str | None = None, when: str | None = None, description: str = ""
    ) -> None:
        guild = interaction.guild
        ...
        if name is None or when is None:
            await interaction.response.send_message(
                voice.decline("Give both a name and a start time to create an event — or neither, to see what's upcoming."),
                ephemeral=True,
            )
            return
    ...
    async def _create(self, interaction: discord.Interaction, name: str, start: str, description: str = "") -> None:
        guild = interaction.guild
    ...
    async def _list_upcoming(self, guild: discord.Guild) -> tuple[discord.Embed, list[events_service.Event]]:
        assert self.bot.db is not None
